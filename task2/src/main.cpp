...
//
// No arguments -> full graded run (correctness smoke test + graded workload
// with a score). You can also check ONE stage in isolation on a workload of
// your choice while developing:
//
...
//     ./bin/matmul all 512 512 512        # every stage on a custom workload
//     (no score)
//     ./bin/matmul help                   # usage
//
// Every stage's result is checked against the naive reference (relative
// tolerance), so a "yes" in the correct column means your kernel is numerically
// right on that input.
...
// (A K-term dot product accumulates rounding, so an absolute 1e-3 would be too
// strict.)
static constexpr float kRelTol = 1e-4f;

// Timing: warmups (untimed) then timed repetitions; the median is reported.
// Matmul kernels are heavier than Task 1's, so fewer reps.
static constexpr int kWarmup = 1;
static constexpr int kReps = 3;

// Default workload and seed. Square M=N=K=kDefN. (Chosen so the working set
// exceeds L2, so cache tiling matters, while the run stays tolerable.)
static constexpr int kDefN = 1024;
static constexpr unsigned kDefSeed = 1234u;

// Scoring: 3 scored stages (simd, prefetch, optimized).
static constexpr double kCorrectPtsPerStage =
    10.0; // -> 3 * 10 = 30 correctness points

struct Tier {
  double speedup;
  double points;
};
// Cumulative tiers for matmul_optimized speedup vs naive (highest reached
// wins). CALIBRATION: tuned so the reference solution reaches the top tier.
// Re-run `make solution && ./bin/matmul_solution` on your lab machines and
// adjust so a correct reference lands at 70.
...
// Stage order is the standard GEMM optimization sequence: a fast
// register-blocked SIMD micro-kernel first, THEN cache-block it and add
// software prefetch. (Cache tiling only pays off once the kernel is fast enough
// to be memory-bound  a scalar loop is compute-bound and tiling would show
// nothing  so SIMD comes first.)
...
// ------------------------------- one workload ------------------------------
// only   : nullptr / "all" -> every stage; otherwise just naive + the named
// stage. show   : print the per-stage timing table. scored : also compute and
// print the autograder score (returned). When !scored, returns the number of
// INCORRECT scored stages that ran.
static double run_config(int M, int N, int K, unsigned seed, const char *only,
  ...
  if (show) {
    std::printf("\n=== Workload  M=%d N=%d K=%d seed=%u %s ===\n", M, N, K,
                seed, scored ? "(GRADED)" : "(not graded)");
    std::printf("%-14s  %-7s  %10s  %10s  %9s\n", "stage", "correct",
                "time(ms)", "GFLOP/s", "speedup");
    std::printf(
        "--------------------------------------------------------------\n");
  }
  ...
  for (int s = 0; s < kNumStages; ++s) {
    ...
    if (!all && !is_naive && std::strcmp(kStages[s].key, only) != 0)
      continue;
  ...
  if (optimized_ok) { // no speedup credit for an incorrect optimized kernel
    for (const Tier &t : kTiers) {
      if (optimized_speedup >= t.speedup)
        tier_pts = t.points;
    }
  }
...
static void usage(const char *prog) {
  ...
  std::printf("M, N, K default to %d; seed defaults to %u. All sizes must be "
              "positive.\n",
              kDefN, kDefSeed);
  std::printf("\nExamples:\n");
  std::printf("  %s simd            %s prefetch 1024 1024 1024        %s all "
              "512 512 512\n",
              prog, prog, prog);
}
...
static const char *match_stage(const char *s) {
  ...
  for (int i = 0; i < kNumStages; ++i)
    if (std::strcmp(s, kStages[i].key) == 0)
      return kStages[i].key;
  return nullptr;
}

int main(int argc, char **argv) {
  std::printf(
      "CS683 PA-1 Task 2  matrix multiplication optimization harness\n");
  ...
  if (argc == 1) {
    ...
    double bad = run_config(256, 256, 256, kDefSeed, nullptr, /*show=*/false,
                            /*scored=*/false);
    if (bad > 0.0)
      std::printf("[smoke] %d scored stage(s) differ from reference on a small "
                  "input.\n",
                  static_cast<int>(bad));
    else
      std::printf("[smoke] all stages match the reference on a small input.\n");

    run_config(kDefN, kDefN, kDefN, kDefSeed, nullptr, /*show=*/true,
               /*scored=*/true);
    std::printf("\nhint: cache blocking's payoff grows with size. Try a bigger "
                "matmul, e.g.\n"
                "      ./bin/matmul all 2048 2048 2048   (watch simd vs "
                "optimized diverge).\n");
    return 0;
  }
